...
abstract final class WeatherPromptBuilder {
  static String? buildSystemSection({
    ...
    buffer.writeln(
      ...
      'Физический комфорт важнее визуального эксперимента: не предлагай лук, в котором пользователю будет холодно/мокро.',
    );
...
ОБЯЗАТЕЛЬНО:
- Если температура <12°C или есть дождь/снег: добавь тёплый верхний слой (верхняя одежда), если есть в гардеробе.
- При дожде/снеге/температуре <14°C: не предлагай открытые сандалии/шлёпки.
- При температуре <10-12°C не собирай образы "футболка + лёгкий низ" без утепляющего слоя.
- Для дождя приоритезируй: более тёмную/практичную палитру, многослойность, защищённую обувь, cozy/comfy силуэт.
- Если в гардеробе нет идеального weather-item, честно скажи об ограничении и собери самый тёплый вариант из доступного.''';
}
